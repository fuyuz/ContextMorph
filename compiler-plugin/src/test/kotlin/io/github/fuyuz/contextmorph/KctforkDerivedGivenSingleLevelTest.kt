...
class KctforkDerivedGivenSingleLevelTest {
    @Test
    fun `derived given is constructed from its dependencies`() {
        val compilationResult = KotlinCompilation().apply {
            ...
            sources = listOf(
                SourceFile.kotlin(
                    ...
                    val result: String =
                        summon<Tag<List<Int>>>().label(listOf(1, 2))
                    """.trimIndent(),
                )
            )
        ...
        val ktClass = compilationResult.classLoader.loadClass("test.DerivedGivenSingleLevelKt")
